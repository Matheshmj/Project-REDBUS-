...
def format_time_column(time_series):
    if pd.api.types.is_timedelta64_dtype(time_series) or pd.api.types.is_string_dtype(time_series):
        return pd.to_datetime(time_series, errors='coerce').dt.strftime('%H:%M:%S')
    return time_series  # If already formatted
...
st.title("REDBUS")
st.image("C:/Users/91848/Desktop/redbus_streamlit/elements/download (1).png", width=700)
